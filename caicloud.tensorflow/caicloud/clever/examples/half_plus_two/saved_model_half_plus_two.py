...
class HalfPlusTwo(dist_base.CaicloudDistTensorflowBase):
  def _write_assets(self, assets_directory, assets_filename):
  ...
  def build_model(self, global_step, is_chief, sync, num_replicas):
    ...
    original_assets_filepath = self._write_assets(original_assets_directory,
    ...
    #####################
    # Args:
    #   sess: 要导出模型所处的 TensorFlow Session。
    #   export_dir: 模型导出路径。
    #   input_tensors: 导出模型的输入的别名和 Tensors 之间的字典，此样例为{'x': x}。
    #   output_tensors: 导出模型的输出的别名和 Tensors 之间的字典，此样例为{'y': y}。
...
if __name__ == "__main__":
  ...
  caicloud_entry.start(HalfPlusTwo)
